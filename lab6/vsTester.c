...
uint16_t tester_sequence_length;
...
void vsTester_init() {
  ...
  tester_sequence_length = 1;
  tester_delay_counter = 0;
  globals_setSequence(vsTester_testSequence, MAX_TEST_SEQUENCE_LENGTH);
  globals_setSequenceIterationLength(tester_sequence_length);
  display_fillScreen(DISPLAY_BLACK);
  vsTester_printInstructions(tester_sequence_length, DRAW_MESSAGE);
}

void vsTester_tick() {
  switch (vsTester_currentState) {
  case vsTester_init_st:
    if (tester_delay_counter < DELAY_COUNTER_EXPIRE) {
      tester_delay_counter++;
    } else {
      tester_delay_counter = 0;
      verifySequence_enable();
      vsTester_printInstructions(tester_sequence_length, ERASE_MESSAGE);
  ...
  case vsTester_running_st:
    if (verifySequence_isComplete()) {
      ...
      if (tester_sequence_length <= MAX_TEST_SEQUENCE_LENGTH) {
        globals_setSequenceIterationLength(tester_sequence_length);
